...
import (
	...
	"github.com/jamescth/MyGoExercise/presentation/protobuf/todo"
)
...
func add(text string) error {
	...
	if err := binary.Write(f, endianness, length(len(b))); err != nil {
		return fmt.Errorf("could not encode length of messag: %v", err)
	}
	...
	if err := f.Close(); err != nil {
		return fmt.Errorf("could not close fiel %s: %v", dbPath, err)
	}
	return nil
}
