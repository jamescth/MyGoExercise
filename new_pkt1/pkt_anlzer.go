...
func ShowPortList() {
	...
	for pidx, pu := range ptList {
		...
		*/
		...
		fmt.Println("")
		if ptList[pidx].synList != nil {
			fmt.Printf(" SYN %d ", ptList[pidx].synList.pkt_idx)

			pz := ptList[pidx].synList.next_syn
			for pz != nil {
				fmt.Printf("%d ", pz.pkt_idx)
				pz = pz.next_syn
			}
		...
		fmt.Println("")
		if ptList[pidx].finList != nil {
			fmt.Printf(" FIN %d ", ptList[pidx].finList.pkt_idx)

			pz := ptList[pidx].finList.next_fin
			for pz != nil {
				fmt.Printf("%d ", pz.pkt_idx)
				pz = pz.next_fin
			}
		...
		fmt.Println("")
		if ptList[pidx].rstList != nil {
			fmt.Printf(" RST %d ", ptList[pidx].rstList.pkt_idx)

			pz := ptList[pidx].rstList.next_rst
			for pz != nil {
				fmt.Printf("%d ", pz.pkt_idx)
				pz = pz.next_rst
			}
		...
		*/
		...
		if p.f_syn {
			if pu.synList == nil {
				...
				p_next.next_syn = p
				Trace.Printf("  p last %d next %d", p_next.pkt_idx, p_next.next_syn.pkt_idx)
			}
		...
		if p.f_fin {
			if pu.finList == nil {
				...
				p_next.next_fin = p
				Trace.Printf("  p last %d next %d", p_next.pkt_idx, p_next.next_fin.pkt_idx)
			}
		...
		if p.f_rst {
			if pu.rstList == nil {
				...
				p_next.next_rst = p
				Trace.Printf("  p last %d next %d", p_next.pkt_idx, p_next.next_rst.pkt_idx)
			}
